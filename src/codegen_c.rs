...
impl CCodegen {
    ...
    fn emit_runtime(&mut self) {
        ...
        if freestanding {
            ...
            self.writeln("static void* rt_malloc(size_t n) { (void)n; return (void*)0; }");
            self.writeln("static void rt_free(void* p) { (void)p; }");
    ...
    fn collect_types(&mut self, program: &Program) {
        fn walk(item: &Item, types: &mut HashSet<String>) {
            match item {
                Item::Attribute(_, inner) => walk(inner, types),
                Item::Namespace(ns) => {
                    for i in &ns.items {
                        walk(i, types);
                    }
    ...
    fn emit_type_body(&mut self, type_name: &str, members: &[Member]) -> CompileResult<()> {
        self.writeln(&format!("typedef struct {}_s {{", type_name));
        self.indent += 1;
        for m in members {
            ...
            if let Member::Property(p) = m {
                if p.auto {
                    self.writeln(&format!("{} {};", c_type(&p.ty), p.name));
                }
            }
        }
        self.indent -= 1;
    ...
    fn emit_class(&mut self, c: &ClassDecl) -> CompileResult<()> {
        self.emit_type_body(&c.name, &c.members)
    }

    fn emit_struct(&mut self, s: &StructDecl) -> CompileResult<()> {
        self.emit_type_body(&s.name, &s.members)
    }
...
fn c_type_in(ty: &TypeRef, types: &HashSet<String>) -> String {
    ...
    if !ty.args.is_empty() && ty.name == "ptr" {
        return format!("{}*", c_type(&ty.args[0]));
    }
    s
}
